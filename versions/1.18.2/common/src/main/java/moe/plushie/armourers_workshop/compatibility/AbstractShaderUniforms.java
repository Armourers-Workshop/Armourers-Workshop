...
public class AbstractShaderUniforms {

    private static final HashMap<Integer, HashMap<String, AbstractShaderUniform>> CHANGED = new HashMap<>();

    private final HashMap<Integer, ArrayList<AbstractShaderUniform>> loadedUniforms = new HashMap<>();

    public static void save(AbstractShaderUniform uniform) {
    ...
    public void apply() {
        ...
        ArrayList<AbstractShaderUniform> uniforms = loadedUniforms.computeIfAbsent(programId, this::getLocations);
        if (uniforms.isEmpty()) {
            return;
        }
        for (AbstractShaderUniform uniform : uniforms) {
            save(uniform);
            uniform.apply();
        }
    }
...
//        lastProgramId = 0;
    ...
    private ArrayList<AbstractShaderUniform> getLocations(int programId) {
        return new AbstractShaderUniform.Loader(programId).uniforms;
    }
