...
public class AbstractForgeEventBus {
    ...
    private static final FastMapper<IEventHandler.Priority, EventPriority> CONVERTER = FastMapper.builder(IEventHandler.Priority.NORMAL, EventPriority.NORMAL, it -> {
        it.put(IEventHandler.Priority.HIGHEST, EventPriority.HIGHEST);
        it.put(IEventHandler.Priority.HIGH, EventPriority.HIGH);
        it.put(IEventHandler.Priority.NORMAL, EventPriority.NORMAL);
        it.put(IEventHandler.Priority.LOW, EventPriority.LOW);
        it.put(IEventHandler.Priority.LOWEST, EventPriority.LOWEST);
    });
    ...
    public static <E extends Event> IEventHandler<E> create(Class<E> eventType) {
        return (priority, receiveCancelled, handler) -> observer(eventType, CONVERTER.getValue(priority), receiveCancelled, handler, event -> event);
    }
}
