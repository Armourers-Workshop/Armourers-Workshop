...
public abstract class ForgeEpicFightShieldRendererMixin {
    ...
    public BakedModel aw2$getItemModel(ItemModelShaper modelShaper, ItemStack arg1, ItemStack itemStack, LivingEntityPatch<?> entityPatch, InteractionHand hand) {
        var model = modelShaper.getItemModel(itemStack);
        var entity = entityPatch.getOriginal();
        var itemModel = ClientWardrobeHandler.getEmbeddedItemModel(itemStack, entity, entity.getLevel(), model);
        if (itemModel != null) {
            var properties = new SkinItemProperties();
            properties.setAllowOverrides(false);
            itemModel.setProperties(properties);
        }
        EmbeddedItemModels.of(itemStack).put(model, itemModel);
        return model;
    }
}
