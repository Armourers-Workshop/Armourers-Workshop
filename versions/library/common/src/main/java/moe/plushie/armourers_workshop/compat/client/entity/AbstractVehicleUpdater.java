...
public class AbstractVehicleUpdater extends AbstractVehicleUpdaterImpl {
    ...
    private void update(IDeltaTracker delta) {
        if (active == null) {
            return;
        }
        ...
        entities.clear();
        oldValue.forEach(it -> apply(it, delta.partialTick(it)));
    }
    ...
    private void apply(Entity entity, float partialTick) {
        ...
        for (int index = 0; index < passengers.size(); index++) {
            ...
            apply(entity, partialTick, index, passenger, attachmentPose);
        }
    }

    private void apply(Entity entity, float partialTick, int index, Entity passenger, SkinAttachmentPose pose) {
