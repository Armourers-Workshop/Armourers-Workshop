...
public abstract class AbstractBlockEntityRenderer<T extends BlockEntity, S extends BlockEntityRenderState> extends AbstractBlockEntityRendererImpl<T> implements IBlockEntityRenderer<T, S> {
    ...
    protected abstract void abi$render(S renderState, int lightmap, int overlay, IGraphicsContext context);
    ...
    @Override
    public final boolean shouldRender(T entity) {
        return abi$shouldRender(entity);
    }

    @Override
    public final boolean shouldRenderOffScreen() {
        return abi$shouldRenderOffScreen();
    }

    @Override
    public final int getViewDistance() {
        return abi$getViewDistance();
    }

    @Override
    public void render(T entity, float f, PoseStack poseStack, MultiBufferSource bufferSource, int lightmap, int j, Vec3 pos) {
        var context = AbstractGraphicsRenderer.wrap(poseStack, bufferSource);
        abi$render(AbstractRenderState.createAndExtract(entity, 0, f), lightmap, OverlayTexture.NO_OVERLAY, context);
    }
}
