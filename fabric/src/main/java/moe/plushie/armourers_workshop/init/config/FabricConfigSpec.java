...
public class FabricConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> {
    ...
    private FabricConfigSpec(UnmodifiableConfig storage, UnmodifiableConfig values, Map<List<String>, String> levelComments) {
        super(storage);
        this.values = values;
        this.levelComments = levelComments;
    }

    protected FabricConfigSpec(UnmodifiableConfig config) {
        super(config);
    }
    ...
    public void setConfig(CommentedConfig config) {
        ...
        if (config != null && !isCorrect(config)) {
            ...
            ModLog.warn("Configuration file {} is not correct. Correcting", configName);
            correct(config,
                    (action, path, incorrectValue, correctedValue) ->
                            ModLog.warn("Incorrect key {} was corrected from {} to its default, {}. {}", DOT_JOINER.join(path), incorrectValue, correctedValue, incorrectValue == correctedValue ? "This seems to be an error." : ""),
                    (action, path, incorrectValue, correctedValue) ->
                            ModLog.debug("The comment on key {} does not match the spec. This may create a backup.", DOT_JOINER.join(path)));
    ...
    public static class Builder {
        ...
        public <T> Pair<T, FabricConfigSpec> configure(Function<Builder, T> consumer) {
        ...
        public FabricConfigSpec build() {
            ...
            FabricConfigSpec ret = new FabricConfigSpec(storage, valueCfg, levelComments);
    ...
    private static class BuilderContext {
        ...
        public void setComment(String... value) {
            this.comment = value == null ? new String[]{""} : value;
        } // TODO 1.17: this should throw an IllegalStateException in future
    ...
    private static class Range<V extends Comparable<? super V>> implements Predicate<Object> {
        ...
        public boolean test(Object t) {
            if (isNumber(t)) {
                ...
                if (!result) {
                    ModLog.debug("Range value {} is not within its bounds {}-{}", n.doubleValue(), ((Number) min).doubleValue(), ((Number) max).doubleValue());
                }
                return result;
            }
            ...
            if (!result) {
                ModLog.debug("Range value {} is not within its bounds {}-{}", c, min, max);
            }
            return result;
        }
