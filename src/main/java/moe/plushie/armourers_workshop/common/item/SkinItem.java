...
public class SkinItem extends Item {
    ...
    public static ArrayList<ITextComponent> getTooltip(ItemStack itemStack) {
        ...
        if (!isOwner) {
            ...
            if (SkinConfig.tooltipSkinName && StringUtils.isNotBlank(skin.getCustomName())) {
                tooltip.add(TranslateUtils.translate("item.armourers_workshop:rollover.skinName", skin.getCustomName().trim()));
            }
        }

        if (SkinConfig.tooltipSkinAuthor && StringUtils.isNotBlank(skin.getAuthorName())) {
        ...
        if (SkinConfig.tooltipFlavour && StringUtils.isNotBlank(skin.getFlavourText())) {
...
//        }
        ...
        if (SkinConfig.tooltipOpenWardrobe) {
            ITextComponent keyName = Keybindings.OPEN_WARDROBE.getTranslatedKeyMessage();
    ...
    public ITextComponent getName(ItemStack itemStack) {
        SkinDescriptor descriptor = SkinDescriptor.of(itemStack);
        if (!descriptor.isEmpty()) {
            return new StringTextComponent(descriptor.getName());
        }
        return new TranslationTextComponent("item.armourers_workshop:rollover.skinInvalidItem");
    }
}
