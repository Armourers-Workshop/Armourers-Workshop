...
public class SkinExporterWavefrontObj implements ISkinExporter {
    ...
    private void exportPart(AWMatrixStack matrixStack, ArrayList<SkinCubeFace> allFaces, SkinPart skinPart, Skin skin, OutputStreamWriter os, TextureBuilder texture, int partIndex) throws IOException {
    ...
    private void exportLayer(AWMatrixStack matrixStack, ArrayList<SkinCubeFace> faces, SkinPart skinPart, Skin skin, OutputStreamWriter os, TextureBuilder texture, String layer, int partIndex) throws IOException {
    ...
    private void writeVert(AWMatrixStack matrixStack, OutputStreamWriter os, float x, float y, float z) throws IOException {
        Vector4f q = new Vector4f(x, y, z, 1);
        matrixStack.applyPose(q);
        os.write(String.format("v %s %s %s", f2s(q.x()), f2s(q.y()), f2s(q.z())) + CRLF);
    }

    private void writeNormal(AWMatrixStack matrixStack, OutputStreamWriter os, float x, float y, float z) throws IOException {
