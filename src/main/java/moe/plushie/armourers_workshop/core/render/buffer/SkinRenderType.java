...
public class SkinRenderType extends RenderType {
    ...
    private static final RenderState.TextureState COLORS = new TextureState(SkinCore.resource("textures/armour/cube.png"), false, false);
    ...
    public static SkinRenderType by(ICube cube) {
        if (cube.isGlass()) {
            if (cube.isGlowing()) {
    ...
    private static RenderType createLightingPart(boolean alpha) {
        // Custom vertex format
        ImmutableList<VertexFormatElement> elements = ImmutableList.<VertexFormatElement>builder()
                .add(DefaultVertexFormats.ELEMENT_POSITION)
                .add(DefaultVertexFormats.ELEMENT_COLOR)
