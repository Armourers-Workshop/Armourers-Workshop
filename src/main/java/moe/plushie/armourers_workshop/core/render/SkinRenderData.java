...
public class SkinRenderData implements SkinBakery.IBakeListener {
    ...
    private int lasTickCount = Integer.MAX_VALUE;
    ...
    public static LazyOptional<SkinRenderData> get(Entity entity) {
        return LazyOptional.of(SkinRenderData::new);
    }
    ...
    protected void invalidateAll() {
        lasTickCount = Integer.MAX_VALUE;
    ...
    private void updateSkin(ItemStack itemStack, boolean allowsArmor) {
        ...
        if (type instanceof ISkinArmorType && allowsArmor) {
            armorSkins.add(new Entry(bakedSkin, descriptor.getColorScheme(), colorScheme));
            loadSkinPart(bakedSkin);
        }
        if (type instanceof ISkinToolType || type == SkinTypes.ITEM) {
            itemSkins.add(new Entry(bakedSkin, descriptor.getColorScheme(), colorScheme));
            loadSkinPart(bakedSkin);
        }
    }
    ...
    public static class Entry {
        ...
        public Entry(BakedSkin bakedSkin, ColorScheme skinScheme, ColorScheme entityScheme) {
            this.bakedSkin = bakedSkin;
            this.bakedScheme = baking(skinScheme, entityScheme);
        }
