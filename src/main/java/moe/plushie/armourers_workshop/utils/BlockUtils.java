...
//    }
    ...
    public static ArrayList<BlockPos> findTouchingBlockFaces(World world, BlockPos pos, Direction facing, int radius, boolean restrictPlane) {
        ...
        while (!openList.isEmpty()) {
            ...
            TileEntity tileEntity = world.getBlockEntity(loc);
    ...
    private static boolean hasPaintableBlock(World world, BlockPos pos) {
        for (int ix = 0; ix < 3; ix++) {
            for (int iy = 0; iy < 3; iy++) {
                for (int iz = 0; iz < 3; iz++) {
                    TileEntity stateValid = world.getBlockEntity(pos.offset(ix - 1, iy - 1, iz - 1));
