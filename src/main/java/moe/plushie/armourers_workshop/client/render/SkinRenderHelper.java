...
public final class SkinRenderHelper {
    ...
    private static void renderGuidePart(ISkinPartType part, float scale, boolean showGuides, SkinProperties skinProps, float r, float g, float b, float a) {
        ...
        if (showGuides) {
            ...
            //
            renderGuideBox(buildRec.getX(), buildRec.getY(), buildRec.getZ(),
                    buildRec.getWidth(), buildRec.getHeight(), buildRec.getDepth(), scale, r, g, b, a);
            //render origin
    ...
    private static void renderGuideFace(EnumFacing dir, double x, double y, double z, double sizeX, double sizeY, float scale, float r, float g, float b, float a) {
        ...
        buff.pos(0, sizeY * scale, 0)
        .tex(sizeY, 0)
        .color(r, g, b, a)
        .endVertex();
        
        buff.pos(sizeX * scale, sizeY * scale, 0)
        .tex(sizeY, sizeX)
        .color(r, g, b, a)
        .endVertex();
        
        buff.pos(sizeX * scale, 0, 0)
        .tex(0, sizeX)
        .color(r, g, b, a)
        .endVertex();
