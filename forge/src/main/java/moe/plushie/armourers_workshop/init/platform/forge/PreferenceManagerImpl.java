...
public class PreferenceManagerImpl {
    ...
    public static Map<String, Object> getServerEntries() {
        return Common.Serializer.snapshot();
    }

    public static void setServerEntries(Map<String, Object> entries) {
        SERVER = entries;
        reloadSpec(COMMON.getRight());
    }

    public static void init() {
        ModLoadingContext.get().registerConfig(net.minecraftforge.fml.config.ModConfig.Type.CLIENT, CLIENT.getRight());
        ModLoadingContext.get().registerConfig(net.minecraftforge.fml.config.ModConfig.Type.COMMON, COMMON.getRight());
    }

    public static void save() {
        Client.Serializer.write(CLIENT.getKey());
        Common.Serializer.write(COMMON.getKey());
    }

    public static void reloadSpec(ForgeConfigSpec spec) {
        ...
        if (COMMON.getValue() == spec) {
            ...
            if (FMLEnvironment.dist.isDedicatedServer() && ModContext.t0() != null) {
                ...
                NetworkManager.sendContextToAll();
            }
        }
    }
    ...
    public static class Client {
        ...
        Client(SpecBuilder builder) {
            ...
            builder.defineCategory("performance", "Change (visual quality/performance) ratio by changing setting in this category.", () -> {
                ...
                bakingCores = MathUtils.clamp(bakingCores, 1, 16);
