...
public class SidebarView extends UIView implements UIScrollViewDelegate {
    ...
    public SidebarView(CGRect frame) {
        super(frame);
        this.iconImage = ModTextures.buttonImage(ModTextures.ADVANCED_SKIN_BUILDER, 0, 0, 24, 24);
        ...
        this.sidebarView.setContents(UIImage.of(ModTextures.ADVANCED_SKIN_BUILDER).uv(0, 0).resizable(24, 24).build());
        ...
        this.containerView.setContents(UIImage.of(ModTextures.ADVANCED_SKIN_BUILDER).uv(0, 24).resizable(24, 24).build());
    ...
    public void reloadData() {
        ...
        for (int j = 0; j < 5; ++j) {
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                iconView.setImage(UIImage.of(ModTextures.TAB_ICONS).uv(16 * 1, 16 * 8).fixed(16, 16).build(), UIControl.State.NORMAL );
