...
import moe.plushie.armourers_workshop.core.network.UpdateColorMixerPacket;
...
public class ColorMixerScreen extends AWAbstractContainerScreen<ColorMixerMenu> {
    ...
    public ColorMixerScreen(ColorMixerMenu container, Inventory inventory, Component title) {
    ...
    protected void renderPreviewColor(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        int cx = leftPos + 108;
        int cy = topPos + 102;
    ...
    private void addPaletteList(int x, int y) {
        ...
        for (Palette palette : PaletteManager.getInstance().getPalettes()) {
            AWComboBox.ComboItem item = new AWComboBox.ComboItem(new TextComponent(palette.getName()));
    ...
    private AWTextField addTextField(int x, int y) {
        AWTextField textBox = new AWTextField(font, x, y, 50, 16, TextComponent.EMPTY);
    ...
    private void addIconButton(int x, int y, int u, int v, String key, Button.OnPress handler) {
    ...
    private void addHelpButton(int x, int y) {
        Component tooltip = getDisplayText("help.palette");
    ...
    public static class Label {
        ...
        public Label(int x, int y, Component title) {
            this.title = title;
            this.x = x;
            this.y = y;
        }
