...
//        return new File(DimensionManager.getCurrentSaveRootDirectory(), "skin-database");
    ...
    public static void recoverSkins(Player player) {
        player.sendMessage(new TextComponent("Starting skin recovery."), player.getUUID());
        ...
        if (skinDir.exists() & skinDir.isDirectory()) {
            ...
            player.sendMessage(new TextComponent(String.format("Found %d skins to be recovered.", skinFiles.length)), player.getUUID());
            player.sendMessage(new TextComponent("Working..."), player.getUUID());
            ...
            player.sendMessage(new TextComponent("Finished skin recovery."), player.getUUID());
            player.sendMessage(new TextComponent(String.format("%d skins were recovered and %d fail recovery.", successCount, failCount)), player.getUUID());
        } else {
            player.sendMessage(new TextComponent("No skins found to recover."), player.getUUID());
        }
    }
    ...
    public static void updateSkins(Player player) {
        ...
        if (!updateDir.exists() & updateDir.isDirectory()) {
            player.sendMessage(new TextComponent("Directory skin-update not found."), player.getUUID());
            return;
        }
        ...
        player.sendMessage(new TextComponent(String.format("Found %d skins to be updated.", skinFiles.length)), player.getUUID());
        player.sendMessage(new TextComponent("Working..."), player.getUUID());
        int successCount = 0;
        int failCount = 0;
        ...
        player.sendMessage(new TextComponent("Finished skin update."), player.getUUID());
        player.sendMessage(new TextComponent(String.format("%d skins were updated and %d failed.", successCount, failCount)), player.getUUID());
    }
