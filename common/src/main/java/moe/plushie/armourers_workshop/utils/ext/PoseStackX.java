...
public abstract class PoseStackX {

    public static PoseStackX create() {
        MatrixX4f pose = MatrixX4f.createScaleMatrix(1, 1, 1);
        MatrixX3f normal = MatrixX3f.createScaleMatrix(1, 1, 1);
        return new PoseStackX() {
            @Override
            public void translate(double x, double y, double z) {
                pose.multiply(MatrixX4f.createTranslateMatrix((float) x, (float) y, (float) z));
            }

            @Override
            public void scale(float x, float y, float z) {
                pose.multiply(MatrixX4f.createScaleMatrix(x, y, z));
                if (x == y && y == z) {
                    if (x > 0.0F) {
                        return;
                    }
                ...
                normal.multiply(MatrixX3f.createScaleMatrix(f3 * f, f3 * f1, f3 * f2));
            }

            @Override
            public void mul(Quaternion quaternion) {
                pose.multiply(new MatrixX4f(quaternion));
                normal.multiply(new MatrixX3f(quaternion));
            }
    ...
    public static PoseStackX wrap(PoseStack matrixStack) {
        return new PoseStackX() {
            @Override
            public void translate(double x, double y, double z) {
