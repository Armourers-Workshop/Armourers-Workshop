...
public class SkinWardrobeScreen extends AWAbstractContainerScreen<SkinWardrobeMenu> {
    ...
    private final AWTabController<SkinWardrobeMenu.Group> tabController = new AWTabController<>(false);
    ...
    public void renderPlayer(PoseStack matrixStack, int mouseX, int mouseY, int x, int y, int width, int height) {
    ...
    protected boolean hasClickedOutside(double mouseX, double mouseY, int left, int top, int button) {
        ...
        if (menu.shouldRenderInventory()) {
            int invLeft = left + 51;
            int invTop = top + 152;
            if (mouseY >= invTop && mouseY <= (invTop + 98)) {
                // inventory doesn't use all the space.
                return mouseX < invLeft || mouseX > (invLeft + 176);
            }
        }
        return false;
    }

    private void switchTab(AWTabController<SkinWardrobeMenu.Group>.Tab tab) {
        menu.setGroup(tab.getTarget());
    }

    private int getExtendedHeight() {
        AWTabController<SkinWardrobeMenu.Group>.Tab tab = tabController.getSelectedTab();
