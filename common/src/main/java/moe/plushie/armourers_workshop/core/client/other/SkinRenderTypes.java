...
public abstract class SkinRenderTypes implements IRenderType {
    ...
    public static IRenderType particle(OpenResourceKey texture, boolean isEmissive, boolean isCull) {
        if (isEmissive) {
    ...
    public static IRenderType image(OpenResourceKey texture) {
    ...
    private static IRenderType _customGeometry(String name, SkinVertexFormat format, OpenResourceKey texture, boolean isTranslucent, boolean isEmissive, boolean isCull) {
    ...
    private static IRenderType _customParticle(String name, SkinVertexFormat format, OpenResourceKey texture, boolean isTranslucent, boolean isEmissive, boolean isCull) {
        var key = String.format("%s/%s", name, texture.path());
    ...
    private static IRenderType _customImage(String name, OpenResourceKey texture) {
        var key = String.format("%s/%s", name, texture.path());
    ...
    private static Builder _entity(SkinVertexFormat format, OpenResourceKey texture) {
        return _builder(format).texture(texture).polygonOffset(0, 30).overlay().lightmap().sortOnUpload().crumbling().outline();
    }

    private static Builder _entityHighlight(OpenResourceKey texture) {
    ...
    private static Builder _blockCutout(OpenResourceKey texture) {
        return _builder(SkinVertexFormat.BLOCK_CUTOUT).texture(texture).overlay().lightmap();
    }

    private static Builder _block(OpenResourceKey texture) {
