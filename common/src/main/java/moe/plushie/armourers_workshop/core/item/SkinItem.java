...
public class SkinItem extends FlavouredBlockItem {
    ...
    protected Component abi$getName(ItemStack itemStack) {
        ...
        if (skin != null) {
            return Component.translatable("item.armourers_workshop.skin.name", TranslateUtils.Name.of(skin.type()));
        }
        return Component.translatable("item.armourers_workshop.skin.name.unknown");
    }
}
