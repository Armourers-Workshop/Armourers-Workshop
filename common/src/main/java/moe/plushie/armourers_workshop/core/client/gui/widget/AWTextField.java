...
public class AWTextField extends AbstractWidget {
    ...
    protected BiFunction<String, Integer, FormattedCharSequence> formatter = (p_195610_0_, p_195610_1_) -> FormattedCharSequence.forward(p_195610_0_, Style.EMPTY);
    ...
    public AWTextField(Font font, int x, int y, int width, int height, Component message) {
    ...
    protected MutableComponent createNarrationMessage() {
        ...
        return new TranslatableComponent("gui.narrate.editBox", itextcomponent, this.value);
    }
    ...
    public void onValueChange(String oldValue, String newValue) {
        ...
        this.nextNarration = System.currentTimeMillis() + 500L;
    }
    ...
    public int getCursorPos(int p_238516_1_) {
        String p_240980_0_ = this.value;
        int p_240980_1_ = this.cursorPos;
        int i = p_240980_0_.length();
        if (p_238516_1_ >= 0) {
            for (int j = 0; p_240980_1_ < i && j < p_238516_1_; ++j) {
            ...
            for (int k = p_238516_1_; p_240980_1_ > 0 && k < 0; ++k) {
                --p_240980_1_;
    ...
    public void setCursorPosition(int p_212422_1_) {
        this.cursorPos = MathUtils.clamp(p_212422_1_, 0, this.value.length());
    ...
    public void setHighlightPos(int p_146199_1_) {
        ...
        this.highlightPos = MathUtils.clamp(p_146199_1_, 0, i);
        if (this.isSingleLine && this.font != null) {
            ...
            this.displayPos = MathUtils.clamp(this.displayPos, 0, i);
        }
    }
    ...
    public void renderLine(PoseStack matrixStack, Line line, int x, int y, float p_230431_4_, int textColor, MultiBufferSource buffers) {
        int dx = x;
        int dy = y + line.offsetY;
        Matrix4f mat = matrixStack.last().pose();

        // first part
        int pos1 = MathUtils.clamp(Math.min(cursorPos, highlightPos) - line.textStart, 0, line.textLength);
        if (pos1 != 0) {
            FormattedCharSequence value = formatter.apply(line.text.substring(0, pos1), line.textStart + pos1);
            dx = font.drawInBatch(value, dx, dy, textColor, true, mat, buffers, false, 0, 15728880) - 1;
        }
        updatePos(line.textStart + pos1, dx, dy);

        // second part
        int pos2 = MathUtils.clamp(Math.max(cursorPos, highlightPos) - line.textStart, 0, line.textLength);
        if (pos2 != pos1) {
            FormattedCharSequence value = formatter.apply(line.text.substring(pos1, pos2), line.textStart + pos2);
            dx = font.drawInBatch(value, dx, dy, textColor, true, mat, buffers, false, 0, 15728880) - 1;
        ...
        if (pos2 != pos3) {
            FormattedCharSequence value = formatter.apply(line.text.substring(pos2, pos3), line.textStart + pos3);
            dx = font.drawInBatch(value, dx, dy, textColor, true, mat, buffers, false, 0, 15728880) - 1;
            updatePos(line.textStart + pos3, dx, dy);
        }
    }

    public void renderButton2(PoseStack matrixStack, int mouseX, int mouseY, float p_230431_4_) {
        ...
        if (flag1) {
            if (flag2) {
                Screen.fill(matrixStack, cursorPosX, cursorPosY - 1, cursorPosX + 1, cursorPosY + lineHeight + 1, -3092272);
            } else {
    ...
    public void renderHighlight2(PoseStack matrixStack, int x, int y) {
        ...
        int offsetX = 0;
        for (Line line : wrappedTextLines) {
            int x0 = MathUtils.clamp(z2 - offsetX, x, x + maxWidth);
            int x1 = MathUtils.clamp(z3 - offsetX, x, x + maxWidth);
            offsetX += maxWidth;
            if (x0 == x1) {
                continue;
            }
    ...
    public void renderButton1(PoseStack p_230431_1_, int p_230431_2_, int p_230431_3_, float p_230431_4_) {
        if (this.isBordered()) {
            int i = this.isFocused() ? -1 : -6250336;
        ...
        if (flag1) {
            if (flag2) {
                Screen.fill(p_230431_1_, k1, i1 - 1, k1 + 1, i1 + 1 + 9, -3092272);
            } else {
    ...
    protected void updateCursorPosWithMouse(double mouseX, double mouseY) {
        int newCursor = 0;
        if (this.isSingleLine) {
            int i = MathUtils.floor(mouseX) - x - contentInset.left;
            ...
            int j = (MathUtils.floor(mouseY) - y - contentInset.top + scrollAmount) / lineHeight;
            int i = MathUtils.floor(mouseX) - x - contentInset.left;
